...
export const useBudgets = (month?: number, year?: number) => {
  ...
  return useQuery({
    ...
    queryFn: () => fetcher<BudgetResponse[]>(`/budgets?month=${m}&year=${y}`),
    staleTime: 1000 * 60 * 5,
...
export const useBudgetPlanning = (month?: number, year?: number) => {
  ...
  return useQuery({
    ...
    queryFn: () => fetcher<BudgetPlanningResponse>(`/budgets/planning?month=${m}&year=${y}`),
    staleTime: 1000 * 60 * 2,
    enabled: !!token, // Only fetch when token exists
  });
};
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ['budget-planning', variables.month, variables.year],
      });
    },
  });
};
