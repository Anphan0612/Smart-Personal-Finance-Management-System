import React, { useCallback, useMemo, useRef, useState, useEffect, useImperativeHandle } from "react";
import {
  View,
  TouchableOpacity,
  TextInput,
...
export const EditTransactionSheet = React.forwardRef<EditTransactionSheetRef, EditTransactionSheetProps>(
  ({ onSave, onClose }, ref) => {
    // Refs
    ...
    // Methods
    const open = useCallback((data: TransactionEditData) => {
      setEditData(data);
      ...
      setSelectedWalletId(data.walletId || useAppStore.getState().activeWalletId || (wallets.length > 0 ? wallets[0].id : null));
    ...
    const isFormValid = Number.isFinite(parsedAmount) && parsedAmount > 0 && !!selectedWalletId && !!selectedCategoryId;
        ...
        <BottomSheetView className="flex-1 px-6">
          ...
          >
            ...
            <View className="items-center py-8">
              ...
              <AtelierTypography variant="caption" className={`mt-2 ${parsedAmount === 0 ? 'text-error font-bold' : 'text-outline'}`}>
                ...
                >
                  ...
                  <View className="w-[47%] p-4 bg-surface-container-low/50 rounded-2xl border border-outline-variant/10">
                    <View className="flex-row items-center gap-2 mb-2">
                      ...
                      <AtelierTypography variant="label" className="text-[10px] text-outline">Cửa hàng</AtelierTypography>
                    </View>
                    <TextInput
                      value={note}
                  ...
                  >
                    <View className="flex-row items-center gap-2 mb-2">
                      ...
                      <AtelierTypography variant="label" className="text-[10px] text-outline">Ngày</AtelierTypography>
                    </View>
                  ...
                  >
                    <View className="flex-row items-center gap-2 mb-2">
                      <Tag size={14} color={!selectedCategoryId ? AtelierTokens.colors.error : "#717785"} />
                      <AtelierTypography variant="label" className={`text-[10px] ${!selectedCategoryId ? 'text-error' : 'text-outline'}`}>Danh mục</AtelierTypography>
                    </View>
                    <View className="flex-row items-center justify-between">
                      <AtelierTypography variant="body" className={`font-bold text-[14px] ${!selectedCategoryId ? 'text-error' : 'text-primary'}`} numberOfLines={1}>
                        {selectedCategory?.name || "Chọn..."}
                      </AtelierTypography>
                      <Edit2 size={12} color={!selectedCategoryId ? AtelierTokens.colors.error : "#005ab4"} />
                    </View>
                  </TouchableOpacity>
                  ...
                  >
                    <View className="flex-row items-center gap-2 mb-2">
                      <WalletIcon size={14} color={!selectedWalletId ? AtelierTokens.colors.error : "#717785"} />
                      <AtelierTypography variant="label" className={`text-[10px] ${!selectedWalletId ? 'text-error' : 'text-outline'}`}>Tài khoản</AtelierTypography>
                    </View>
                    <View className="flex-row items-center justify-between">
                      <AtelierTypography variant="body" className={`font-bold text-[14px] ${!selectedWalletId ? 'text-error' : 'text-surface-on'}`} numberOfLines={1}>
                        {selectedWallet?.name || "Chọn..."}
                      </AtelierTypography>
                      <Edit2 size={12} color={!selectedWalletId ? AtelierTokens.colors.error : "#717785"} />
                    </View>
                  </TouchableOpacity>
                </MotiView>
              ) : (
                ...
                >
                  <View className="flex-row items-center justify-between mb-4">
                    ...
                    <TouchableOpacity onPress={() => setActivePicker("none")}>
                      <AtelierTypography variant="label" className="text-outline">Quay lại</AtelierTypography>
                    </TouchableOpacity>
                  </View>
                      ...
                      {(activePicker === "category" ? categories : wallets).map((item: any) => (
                        <TouchableOpacity
                          ...
                          className={`px-4 py-3 rounded-xl border ${
                            (activePicker === "category" ? selectedCategoryId : selectedWalletId) === item.id
                        ...
                        >
                          <AtelierTypography
                            variant="label"
                            className={ (activePicker === "category" ? selectedCategoryId : selectedWalletId) === item.id ? "text-white" : "text-surface-on" }
                          >
                            {item.name}
                          </AtelierTypography>
                        </TouchableOpacity>
            ...
            <View className="mt-12 mb-8">
              ...
              >
                <AtelierTypography variant="label" className="text-outline">Hủy bỏ</AtelierTypography>
              </TouchableOpacity>
            </View>
          </ScrollView>
          </KeyboardAvoidingView>
