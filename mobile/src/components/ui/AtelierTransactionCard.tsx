...
import { Coffee, Tag, ShoppingBag, Utensils, Home, Car, Zap, Heart, Edit3, Check } from "lucide-react-native";
...
export const AtelierTransactionCard = ({ 
  ...
  const containerProps = isBubble 
    ? { 
        ...
        padding: "md" as const, 
        className: "w-full bg-surface-container-lowest border border-outline-variant/10 rounded-[24px] shadow-sm shadow-black/5" 
      } 
    : { 
        elevation: "lowest" as const, 
        padding: "md" as const, 
    ...
    <Container {...containerProps}>
      ...
      <View className="flex-row items-center justify-between mb-4 gap-2">
        <View className="flex-row items-center gap-3 flex-1">
          ...
          <View className="flex-1 pr-2">
            <AtelierTypography variant="h3" className="text-[15px] font-bold text-surface-on" numberOfLines={1}>
              {data.note || "Giao dịch mới"}
            </AtelierTypography>
            <AtelierTypography variant="label" className="text-[10px] text-surface-on-variant uppercase tracking-[1.5px] font-bold opacity-60 mt-0.5">
              {data.type === "EXPENSE" ? "Khoản chi" : "Khoản thu"}
            </AtelierTypography>
          </View>
        </View>
        <View className={`px-2.5 py-1 rounded-full shrink-0 border ${isConfirmed ? 'bg-ai-primary/10 border-ai-primary/20' : 'bg-surface-container-high border-outline-variant/20'}`}>
          <AtelierTypography variant="label" className={`text-[9px] font-bold uppercase tracking-wider ${isConfirmed ? 'text-ai-primary' : 'text-surface-on-variant'}`}>
      ...
      <View className="bg-surface-container-low rounded-2xl p-4 mb-4 border border-outline-variant/10 shadow-sm shadow-black/5">
        ...
        <View className="flex-row items-baseline justify-between mb-4 pb-4 border-b border-outline-variant/10">
          <AtelierTypography variant="label" className="text-[10px] text-surface-on-variant font-bold uppercase tracking-widest opacity-50">SỐ TIỀN</AtelierTypography>
          <AtelierTypography variant="h2" className={`text-[20px] font-bold ${data.type === 'EXPENSE' ? 'text-error' : 'text-primary'}`}>
        ...
        <View className="flex-row justify-between">
          <TouchableOpacity 
            ...
            disabled={isPending}
          >
            <AtelierTypography variant="label" className="text-[10px] uppercase font-bold text-surface-on-variant opacity-50 tracking-widest mb-1.5">
              DANH MỤC
            </AtelierTypography>
            <View className="flex-row items-center gap-1.5">
              ...
              <Edit3 size={12} color={!data.categoryId ? AtelierTokens.colors.error : AtelierTokens.colors.outline} />
            </View>
          </TouchableOpacity>
          ...
          <>
            ...
            >
              {isPending ? (
                ...
                <AtelierTypography variant="label" className="text-white text-[14px] font-bold normal-case">
                  Xác nhận
                </AtelierTypography>
              )}
            </TouchableOpacity>
            ...
            >
              <AtelierTypography variant="label" className="text-surface-on text-[14px] font-bold normal-case">
                Sửa chi tiết
              </AtelierTypography>
            </TouchableOpacity>
          </>
