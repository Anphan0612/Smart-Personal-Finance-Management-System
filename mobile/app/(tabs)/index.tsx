...
export default function HomeScreen() {
  ...
  const totalBudgetLimit = budgets?.reduce((s, b) => s + b.limitAmount, 0) ?? 0;
  const totalBudgetSpent = budgets?.reduce((s, b) => s + b.currentSpending, 0) ?? 0;
      ...
      <View className="mb-4">
        ...
        <View className="gap-3">
          {isLoading ? (
            ...
            dashboard?.transactions?.map((item, idx) => (
              <MotiView
                key={item.id}
                from={{ opacity: 0, translateY: 10 }}
                animate={{ opacity: 1, translateY: 0 }}
